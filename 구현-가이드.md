# VM 오토스케일링 구현 상세 가이드

**작성일**: 2025-11-20  
**버전**: 1.0

---

## 📋 목차

1. [환경 설정](#환경-설정)
2. [템플릿 관리 구현](#템플릿-관리-구현)
3. [오토스케일링 설정 구현](#오토스케일링-설정-구현)
4. [PLG Stack 연동 구현](#plg-stack-연동-구현)
5. [Jenkins 파이프라인 구현](#jenkins-파이프라인-구현)
6. [F5 자동 연동 구현](#f5-자동-연동-구현)
7. [IP Pool 관리 구현](#ip-pool-관리-구현)

---

## 환경 설정

### 1. 필수 도구 설치

```bash
# govc 설치 (vCenter API 사용)
brew install govc

# 또는 직접 다운로드
wget https://github.com/vmware/govmomi/releases/download/v0.30.0/govc_darwin_amd64.gz
gunzip govc_darwin_amd64.gz
chmod +x govc_darwin_amd64
sudo mv govc_darwin_amd64 /usr/local/bin/govc
```

### 2. 환경 변수 설정

```bash
# vCenter 연결 정보
export GOVC_URL=https://10.255.0.10
export GOVC_USERNAME=administrator@vsphere.local
export GOVC_PASSWORD=VMware1!

# Jenkins 서버 접속 정보
export JENKINS_SERVER=10.255.0.103
export JENKINS_SSH_USER=jenkins
export JENKINS_SSH_PASSWORD=!danacloud12
export JENKINS_WEB_USER=danacloud
export JENKINS_WEB_PASSWORD=!danacloud12

# Jenkins 연결 정보
export JENKINS_URL=http://10.255.0.103:8080
export JENKINS_USER=jenkins
export JENKINS_TOKEN=<jenkins-api-token>

# F5 연결 정보
export F5_SERVERS=10.255.1.80,10.255.1.81
export F5_USERNAME=admin
export F5_PASSWORD=<f5-password>

# PLG Stack 서버 접속 정보
export PLG_STACK_SERVER=10.255.1.254
export PLG_STACK_USER=ubuntu
export PLG_STACK_SSH_KEY=/Users/jhlee/Desktop/Work/Dana-Cloud-Oper/DanaIX-신규\ VM\ 생성/00.\ SSH-PemKey-List/danainfra

# PLG Stack 연결 정보 (실제 서버 정보)
export PROMETHEUS_URL=http://10.255.1.254:9090
export ALERTMANAGER_URL=http://10.255.1.254:9093
export GRAFANA_URL=http://10.255.1.254:3000
export LOKI_URL=http://10.255.1.254:3100

# Jenkins Webhook 정보 (Alertmanager에서 호출)
export JENKINS_WEBHOOK_URL=http://10.255.0.103:8080/generic-webhook-trigger/invoke?token=plg-autoscale-token
export JENKINS_WEBHOOK_USER=danacloud
export JENKINS_WEBHOOK_PASSWORD=!danacloud12
```

### 3. PLG Stack 서버 정보

#### 서버 접속 정보
- **서버 IP**: `10.255.1.254`
- **사용자**: `ubuntu`
- **SSH Key 경로**: `/Users/jhlee/Desktop/Work/Dana-Cloud-Oper/DanaIX-신규 VM 생성/00. SSH-PemKey-List/danainfra`

#### 서버 접속 방법
```bash
# SSH 접속
ssh -i '/Users/jhlee/Desktop/Work/Dana-Cloud-Oper/DanaIX-신규 VM 생성/00. SSH-PemKey-List/danainfra' ubuntu@10.255.1.254
```

#### PLG Stack 구성 요소

| 구성 요소 | 포트 | 접속 URL | 설명 |
|---------|------|----------|------|
| **Prometheus** | 9090 | http://10.255.1.254:9090 | 메트릭 수집 및 Alert Rule 평가 |
| **Alertmanager** | 9093 | http://10.255.1.254:9093 | 알림 관리 및 Webhook 전송 |
| **Grafana** | 3000 | http://10.255.1.254:3000 | 대시보드 및 시각화 |
| **Loki** | 3100 | http://10.255.1.254:3100 | 로그 수집 (선택사항) |

#### Docker 컨테이너 정보
```bash
# 컨테이너 목록 확인
sudo docker ps

# 예상 출력:
# NAMES          IMAGE                      PORTS
# grafana        grafana/grafana:latest     0.0.0.0:3000->3000/tcp
# prometheus     prom/prometheus:latest     0.0.0.0:9090->9090/tcp
# alertmanager   prom/alertmanager:latest    0.0.0.0:9093->9093/tcp
# loki           grafana/loki:latest        0.0.0.0:3100->3100/tcp
```

#### 설정 파일 위치

| 구성 요소 | 설정 파일 경로 (호스트) | 컨테이너 내부 경로 |
|---------|----------------------|------------------|
| **Prometheus** | `/mnt/plg-stack/prometheus/prometheus.yml` | `/etc/prometheus/prometheus.yml` |
| **Alert Rules** | `/mnt/plg-stack/prometheus/rules/alert_rules.yml` | `/etc/prometheus/rules/alert_rules.yml` |
| **Alertmanager** | `/mnt/plg-stack/alertmanager/config/alertmanager.yml` | `/etc/alertmanager/alertmanager.yml` |

#### 설정 파일 수정 방법
```bash
# 1. 서버 접속
ssh -i '/Users/jhlee/Desktop/Work/Dana-Cloud-Oper/DanaIX-신규 VM 생성/00. SSH-PemKey-List/danainfra' ubuntu@10.255.1.254

# 2. Prometheus 설정 수정
sudo vi /mnt/plg-stack/prometheus/prometheus.yml

# 3. Alert Rules 수정
sudo vi /mnt/plg-stack/prometheus/rules/alert_rules.yml

# 4. Alertmanager 설정 수정
sudo vi /mnt/plg-stack/alertmanager/config/alertmanager.yml

# 5. 컨테이너 재시작 (설정 적용)
sudo docker restart prometheus
sudo docker restart alertmanager
```

#### 현재 Prometheus 설정 요약
- **Scrape Interval**: 15초
- **Evaluation Interval**: 15초
- **모니터링 대상**:
  - `prometheus` (자체 모니터링)
  - `alertmanager` (Alertmanager 모니터링)
  - `loki` (Loki 모니터링)
  - `nginx-test-01` (10.255.1.253:9100)
  - `nginx-test-02` (10.255.1.102:9100)

#### 현재 Alertmanager 설정 요약
- **Webhook URL**: `http://10.255.0.103:8080/generic-webhook-trigger/invoke?token=plg-autoscale-token`
- **Webhook 인증**: Basic Auth (`danacloud` / `!danacloud12`)
- **라우팅 규칙**: `severity: critical` 또는 `severity: warning` 알림을 Jenkins Webhook으로 전송
- **Group Wait**: 10초
- **Repeat Interval**: 12시간

#### 현재 Alert Rules 요약
- **Alert Rules 파일**: `/mnt/plg-stack/prometheus/rules/alert_rules.yml`
- **그룹**:
  - `vm-autoscale`: VM 오토스케일링 관련 규칙 (현재 테스트용으로 비활성화)
  - `nginx-test-autoscale`: nginx-test 서버 모니터링 규칙 (테스트용으로 비활성화)
- **주요 Alert**:
  - `HighCPU`: CPU 사용률 임계치 (현재 비활성화)
  - `HighMemory`: Memory 사용률 임계치 (현재 비활성화)
  - `HighDisk`: Disk 사용률 임계치 (현재 비활성화)

### 4. Jenkins 서버 정보

#### 서버 접속 정보
- **서버 IP**: `10.255.0.103`
- **SSH 접속**:
  - 사용자: `jenkins`
  - 비밀번호: `!danacloud12`
  - Sudo 비밀번호: `!danacloud12`
- **Web 접속**:
  - URL: `http://10.255.0.103:8080/`
  - 사용자: `danacloud`
  - 비밀번호: `!danacloud12`

#### 서버 접속 방법
```bash
# SSH 접속
ssh jenkins@10.255.0.103
# 비밀번호: !danacloud12

# 또는 sshpass 사용
sshpass -p '!danacloud12' ssh jenkins@10.255.0.103
```

#### Jenkins 기본 정보
- **Jenkins 버전**: `2.528.1`
- **Jenkins Home**: `/var/lib/jenkins`
- **Jenkins Port**: `8080` (IPv6: `:::8080`)
- **Jenkins 프로세스**: Java 프로세스로 실행 중
- **Jenkins 사용자**: `jenkins`
- **Jenkins 그룹**: `jenkins`

#### Jenkins Jobs 목록
| Job 이름 | 설명 | 타입 |
|---------|------|------|
| **plg-autoscale-out** | Alertmanager Webhook → vCenter API → F5 Pool 자동 등록 파이프라인 | Pipeline |
| **vm-deployment-pipeline** | Terraform을 사용한 VM 자동 배포 파이프라인 | Pipeline |
| **golden-image-build** | Packer를 사용한 골든 이미지 빌드 파이프라인 | Pipeline |

#### 설치된 주요 플러그인
- **generic-webhook-trigger** (v2.4.1): Generic Webhook Trigger 플러그인
- **workflow-job**: Pipeline Job 지원
- **git**: Git 연동
- **git-client**: Git 클라이언트
- **github**: GitHub 연동
- **pipeline-build-step**: Pipeline 빌드 단계

#### Jenkins 설정 파일 위치
| 설정 항목 | 파일 경로 |
|---------|----------|
| **Jenkins 메인 설정** | `/var/lib/jenkins/config.xml` |
| **Credentials** | `/var/lib/jenkins/credentials.xml` |
| **Secrets** | `/var/lib/jenkins/secrets/` |
| **Job 설정** | `/var/lib/jenkins/jobs/{job-name}/config.xml` |
| **Workspace** | `/var/lib/jenkins/workspace/{job-name}/` |

#### Generic Webhook Trigger 설정

**plg-autoscale-out Job의 Webhook 설정**:
- **Webhook URL**: `http://10.255.0.103:8080/generic-webhook-trigger/invoke?token=plg-autoscale-token`
- **Token**: `plg-autoscale-token`
- **파싱 변수**:
  - `alertname`: `$.alerts[0].labels.alertname`
  - `instance`: `$.alerts[0].labels.instance`
  - `severity`: `$.alerts[0].labels.severity`
  - `status`: `$.status`
  - `summary`: `$.alerts[0].annotations.summary`
  - `description`: `$.alerts[0].annotations.description`
- **필터**: `status == "firing"` (firing 상태의 알림만 처리)

#### Jenkins Job 설정 확인 방법
```bash
# 1. 서버 접속
ssh jenkins@10.255.0.103
# 비밀번호: !danacloud12

# 2. Job 설정 파일 확인
echo "!danacloud12" | sudo -S cat /var/lib/jenkins/jobs/plg-autoscale-out/config.xml

# 3. Job 빌드 로그 확인
echo "!danacloud12" | sudo -S ls -la /var/lib/jenkins/jobs/plg-autoscale-out/builds/

# 4. 최신 빌드 로그 확인
echo "!danacloud12" | sudo -S find /var/lib/jenkins/jobs/plg-autoscale-out/builds -name "log" -type f | sort -r | head -1 | xargs -I {} echo "!danacloud12" | sudo -S tail -100 {}
```

#### Jenkins 서비스 관리
```bash
# Jenkins 서비스 상태 확인
echo "!danacloud12" | sudo -S systemctl status jenkins

# Jenkins 서비스 재시작
echo "!danacloud12" | sudo -S systemctl restart jenkins

# Jenkins 서비스 시작
echo "!danacloud12" | sudo -S systemctl start jenkins

# Jenkins 서비스 중지
echo "!danacloud12" | sudo -S systemctl stop jenkins
```

#### Jenkins API 사용
```bash
# Jenkins API 토큰 생성 (Web UI에서)
# Manage Jenkins → Manage Users → danacloud → Configure → API Token 생성

# API 호출 예시
curl -u danacloud:!danacloud12 http://10.255.0.103:8080/api/json

# Job 실행
curl -X POST -u danacloud:!danacloud12 \
  "http://10.255.0.103:8080/job/plg-autoscale-out/build?token=plg-autoscale-token"
```

### 5. 데이터 저장소 설정

```bash
# 데이터 디렉토리 생성
mkdir -p /Users/jhlee/Desktop/Work/Dana-Cloud-Oper/DanaIX-Infra-Automation/VM-Autoscaling/data
```

---

## 템플릿 관리 구현

### 1. 백엔드 서비스 구현

#### 파일: `backend/src/services/templateService.js`

```javascript
const { exec } = require('child_process');
const util = require('util');
const execPromise = util.promisify(exec);
const fs = require('fs').promises;
const path = require('path');

const TEMPLATES_DATA_PATH = path.join(__dirname, '../../data/templates.json');
const VCENTER_URL = process.env.GOVC_URL;
const VCENTER_USERNAME = process.env.GOVC_USERNAME;
const VCENTER_PASSWORD = process.env.GOVC_PASSWORD;

/**
 * VM을 템플릿으로 변환
 * @param {string} vmId - VM ID 또는 이름
 * @param {string} templateName - 템플릿 이름
 * @returns {Promise<object>} 변환 결과
 */
async function convertVmToTemplate(vmId, templateName) {
  try {
    // govc 명령어로 VM을 템플릿으로 변환
    const command = `govc vm.markastemplate -vm="${vmId}" "${templateName}"`;
    
    await execPromise(command, {
      env: {
        ...process.env,
        GOVC_URL: VCENTER_URL,
        GOVC_USERNAME: VCENTER_USERNAME,
        GOVC_PASSWORD: VCENTER_PASSWORD
      }
    });

    return {
      success: true,
      templateName: templateName,
      message: `VM '${vmId}'을 템플릿 '${templateName}'으로 변환했습니다.`
    };
  } catch (error) {
    console.error(`[Template Service] 템플릿 변환 실패:`, error);
    throw new Error(`템플릿 변환 실패: ${error.message}`);
  }
}

/**
 * 템플릿 메타데이터 저장
 * @param {object} templateData - 템플릿 데이터
 * @returns {Promise<object>} 저장된 템플릿 정보
 */
async function saveTemplateMetadata(templateData) {
  try {
    // 기존 템플릿 목록 로드
    let templates = [];
    try {
      const data = await fs.readFile(TEMPLATES_DATA_PATH, 'utf8');
      templates = JSON.parse(data);
    } catch (err) {
      // 파일이 없으면 빈 배열로 시작
      templates = [];
    }

    // 새 템플릿 추가
    const newTemplate = {
      id: `template-${Date.now()}`,
      ...templateData,
      createdAt: new Date().toISOString()
    };
    templates.push(newTemplate);

    // 파일에 저장
    await fs.writeFile(TEMPLATES_DATA_PATH, JSON.stringify(templates, null, 2));

    return newTemplate;
  } catch (error) {
    console.error(`[Template Service] 템플릿 메타데이터 저장 실패:`, error);
    throw new Error(`템플릿 메타데이터 저장 실패: ${error.message}`);
  }
}

/**
 * 템플릿 목록 조회
 * @returns {Promise<Array>} 템플릿 목록
 */
async function getTemplates() {
  try {
    const data = await fs.readFile(TEMPLATES_DATA_PATH, 'utf8');
    return JSON.parse(data);
  } catch (error) {
    if (error.code === 'ENOENT') {
      return [];
    }
    throw error;
  }
}

/**
 * 템플릿 상세 조회
 * @param {string} templateId - 템플릿 ID
 * @returns {Promise<object>} 템플릿 정보
 */
async function getTemplateById(templateId) {
  const templates = await getTemplates();
  return templates.find(t => t.id === templateId);
}

/**
 * 템플릿 삭제
 * @param {string} templateId - 템플릿 ID
 * @returns {Promise<object>} 삭제 결과
 */
async function deleteTemplate(templateId) {
  try {
    const templates = await getTemplates();
    const template = templates.find(t => t.id === templateId);
    
    if (!template) {
      throw new Error(`템플릿을 찾을 수 없습니다: ${templateId}`);
    }

    // vCenter에서 템플릿 삭제 (선택사항)
    // await execPromise(`govc object.destroy "${template.vcenterTemplateName}"`);

    // 메타데이터에서 제거
    const updatedTemplates = templates.filter(t => t.id !== templateId);
    await fs.writeFile(TEMPLATES_DATA_PATH, JSON.stringify(updatedTemplates, null, 2));

    return {
      success: true,
      message: `템플릿 '${template.name}'을 삭제했습니다.`
    };
  } catch (error) {
    console.error(`[Template Service] 템플릿 삭제 실패:`, error);
    throw error;
  }
}

module.exports = {
  convertVmToTemplate,
  saveTemplateMetadata,
  getTemplates,
  getTemplateById,
  deleteTemplate
};
```

### 2. API 엔드포인트 구현

#### 파일: `backend/src/server.js` (추가)

```javascript
const templateService = require('./services/templateService');

// 템플릿 생성
if (parsedUrl.pathname === '/api/templates' && req.method === 'POST') {
  let body = '';
  req.on('data', chunk => { body += chunk.toString(); });
  req.on('end', async () => {
    try {
      const payload = JSON.parse(body);
      
      // VM을 템플릿으로 변환
      const convertResult = await templateService.convertVmToTemplate(
        payload.sourceVmId,
        payload.name
      );

      // 템플릿 메타데이터 저장
      const template = await templateService.saveTemplateMetadata({
        name: payload.name,
        description: payload.description,
        vcenterTemplateName: payload.name,
        sourceVmId: payload.sourceVmId,
        sourceVmName: payload.sourceVmName
      });

      sendJSONResponse(res, 200, {
        success: true,
        template: template
      });
    } catch (error) {
      sendJSONResponse(res, 500, {
        success: false,
        error: error.message
      });
    }
  });
  return;
}

// 템플릿 목록 조회
if (parsedUrl.pathname === '/api/templates' && req.method === 'GET') {
  try {
    const templates = await templateService.getTemplates();
    sendJSONResponse(res, 200, {
      success: true,
      templates: templates
    });
  } catch (error) {
    sendJSONResponse(res, 500, {
      success: false,
      error: error.message
    });
  }
  return;
}
```

---

## 오토스케일링 설정 구현

### 1. 백엔드 서비스 구현

#### 파일: `backend/src/services/autoscalingService.js`

```javascript
const fs = require('fs').promises;
const path = require('path');

const CONFIGS_DATA_PATH = path.join(__dirname, '../../data/autoscaling-configs.json');

/**
 * 오토스케일링 설정 저장
 * @param {object} configData - 설정 데이터
 * @returns {Promise<object>} 저장된 설정 정보
 */
async function saveConfig(configData) {
  try {
    let configs = [];
    try {
      const data = await fs.readFile(CONFIGS_DATA_PATH, 'utf8');
      configs = JSON.parse(data);
    } catch (err) {
      configs = [];
    }

    const newConfig = {
      id: `autoscale-${Date.now()}`,
      ...configData,
      enabled: configData.enabled !== undefined ? configData.enabled : true,
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString()
    };
    configs.push(newConfig);

    await fs.writeFile(CONFIGS_DATA_PATH, JSON.stringify(configs, null, 2));
    return newConfig;
  } catch (error) {
    console.error(`[Autoscaling Service] 설정 저장 실패:`, error);
    throw error;
  }
}

/**
 * 오토스케일링 설정 목록 조회
 * @returns {Promise<Array>} 설정 목록
 */
async function getConfigs() {
  try {
    const data = await fs.readFile(CONFIGS_DATA_PATH, 'utf8');
    return JSON.parse(data);
  } catch (error) {
    if (error.code === 'ENOENT') {
      return [];
    }
    throw error;
  }
}

/**
 * 오토스케일링 설정 상세 조회
 * @param {string} configId - 설정 ID
 * @returns {Promise<object>} 설정 정보
 */
async function getConfigById(configId) {
  const configs = await getConfigs();
  return configs.find(c => c.id === configId);
}

/**
 * 오토스케일링 설정 수정
 * @param {string} configId - 설정 ID
 * @param {object} updateData - 수정할 데이터
 * @returns {Promise<object>} 수정된 설정 정보
 */
async function updateConfig(configId, updateData) {
  try {
    const configs = await getConfigs();
    const index = configs.findIndex(c => c.id === configId);
    
    if (index === -1) {
      throw new Error(`설정을 찾을 수 없습니다: ${configId}`);
    }

    configs[index] = {
      ...configs[index],
      ...updateData,
      updatedAt: new Date().toISOString()
    };

    await fs.writeFile(CONFIGS_DATA_PATH, JSON.stringify(configs, null, 2));
    return configs[index];
  } catch (error) {
    console.error(`[Autoscaling Service] 설정 수정 실패:`, error);
    throw error;
  }
}

/**
 * 오토스케일링 활성화/비활성화
 * @param {string} configId - 설정 ID
 * @param {boolean} enabled - 활성화 여부
 * @returns {Promise<object>} 수정된 설정 정보
 */
async function setConfigEnabled(configId, enabled) {
  return await updateConfig(configId, { enabled });
}

module.exports = {
  saveConfig,
  getConfigs,
  getConfigById,
  updateConfig,
  setConfigEnabled
};
```

---

## PLG Stack 연동 구현

### 1. PLG Stack 서버 접속 및 설정 파일 수정

#### 서버 접속
```bash
# PLG Stack 서버 접속
ssh -i '/Users/jhlee/Desktop/Work/Dana-Cloud-Oper/DanaIX-신규 VM 생성/00. SSH-PemKey-List/danainfra' ubuntu@10.255.1.254
```

#### 설정 파일 위치
- **Prometheus 설정**: `/mnt/plg-stack/prometheus/prometheus.yml`
- **Alert Rules**: `/mnt/plg-stack/prometheus/rules/alert_rules.yml`
- **Alertmanager 설정**: `/mnt/plg-stack/alertmanager/config/alertmanager.yml`

#### 설정 파일 수정 후 컨테이너 재시작
```bash
# Prometheus 재시작
sudo docker restart prometheus

# Alertmanager 재시작
sudo docker restart alertmanager
```

### 2. Prometheus Alert Rule 생성

#### 파일: `backend/src/services/prometheusService.js`

```javascript
const axios = require('axios');
const { exec } = require('child_process');
const util = require('util');
const execPromise = util.promisify(exec);
const fs = require('fs').promises;

const PROMETHEUS_URL = process.env.PROMETHEUS_URL || 'http://10.255.1.254:9090';
const PLG_STACK_SERVER = process.env.PLG_STACK_SERVER || '10.255.1.254';
const PLG_STACK_USER = process.env.PLG_STACK_USER || 'ubuntu';
const PLG_STACK_SSH_KEY = process.env.PLG_STACK_SSH_KEY || '/Users/jhlee/Desktop/Work/Dana-Cloud-Oper/DanaIX-신규 VM 생성/00. SSH-PemKey-List/danainfra';
const ALERT_RULES_PATH = '/mnt/plg-stack/prometheus/rules/alert_rules.yml';

/**
 * Prometheus Alert Rule 생성
 * @param {object} config - 오토스케일링 설정
 * @returns {Promise<object>} 생성 결과
 */
async function createAlertRule(config) {
  const alertRule = {
    alert: `${config.serviceName}_HighResourceUsage`,
    expr: `
      (
        avg(rate(node_cpu_seconds_total{job="${config.monitoring.prometheusJob}"}[5m])) * 100 > ${config.monitoring.cpuThreshold}
        OR
        avg(node_memory_MemTotal_bytes{job="${config.monitoring.prometheusJob}"} - node_memory_MemAvailable_bytes{job="${config.monitoring.prometheusJob}"}) / node_memory_MemTotal_bytes{job="${config.monitoring.prometheusJob}"} * 100 > ${config.monitoring.memoryThreshold}
      )
      AND
      (
        avg_over_time(rate(node_cpu_seconds_total{job="${config.monitoring.prometheusJob}"}[5m])[${config.monitoring.duration}m:]) * 100 > ${config.monitoring.cpuThreshold}
        OR
        avg_over_time((node_memory_MemTotal_bytes{job="${config.monitoring.prometheusJob}"} - node_memory_MemAvailable_bytes{job="${config.monitoring.prometheusJob}"}) / node_memory_MemTotal_bytes{job="${config.monitoring.prometheusJob}"}[${config.monitoring.duration}m:]) * 100 > ${config.monitoring.memoryThreshold}
      )
    `,
    for: `${config.monitoring.duration}m`,
    labels: {
      severity: 'warning',
      service: config.serviceName,
      autoscaleConfigId: config.id
    },
    annotations: {
      summary: `${config.serviceName} 리소스 사용률이 높습니다`,
      description: `CPU 또는 Memory 사용률이 ${config.monitoring.cpuThreshold}%를 초과하여 ${config.monitoring.duration}분 이상 지속되었습니다.`
    }
  };

  // PLG Stack 서버의 Alert Rules 파일에 추가
  try {
    // SSH로 서버에 접속하여 Alert Rules 파일 수정
    const sshCommand = `ssh -i "${PLG_STACK_SSH_KEY}" -o StrictHostKeyChecking=no ${PLG_STACK_USER}@${PLG_STACK_SERVER} "cat ${ALERT_RULES_PATH}"`;
    const currentRules = await execPromise(sshCommand);
    
    // YAML 파싱 및 새 규칙 추가 (실제 구현 시 yaml 라이브러리 사용)
    // 여기서는 간단한 예시만 제공
    
    // 새 규칙을 파일에 추가하는 명령어
    const addRuleCommand = `ssh -i "${PLG_STACK_SSH_KEY}" -o StrictHostKeyChecking=no ${PLG_STACK_USER}@${PLG_STACK_SERVER} << 'EOF'
cat >> ${ALERT_RULES_PATH} << 'ALERTRULE'
  - alert: ${alertRule.alert}
    expr: ${alertRule.expr}
    for: ${alertRule.for}
    labels:
      severity: ${alertRule.labels.severity}
      service: ${alertRule.labels.service}
      autoscaleConfigId: ${alertRule.labels.autoscaleConfigId}
    annotations:
      summary: ${alertRule.annotations.summary}
      description: ${alertRule.annotations.description}
ALERTRULE
sudo docker restart prometheus
EOF
`;
    
    await execPromise(addRuleCommand);
    
    return {
      success: true,
      alertRule: alertRule,
      message: 'Alert Rule이 추가되었습니다.'
    };
  } catch (error) {
    console.error(`[Prometheus Service] Alert Rule 추가 실패:`, error);
    throw new Error(`Alert Rule 추가 실패: ${error.message}`);
  }
}

/**
 * Prometheus 설정에 새 Job 추가
 * @param {object} config - 오토스케일링 설정
 * @returns {Promise<object>} 설정 결과
 */
async function addPrometheusJob(config) {
  const prometheusConfigPath = '/mnt/plg-stack/prometheus/prometheus.yml';
  const sshCommand = `ssh -i "${PLG_STACK_SSH_KEY}" -o StrictHostKeyChecking=no ${PLG_STACK_USER}@${PLG_STACK_SERVER}`;
  
  // Prometheus 설정 파일에 새 scrape_config 추가
  // 실제 구현 시 YAML 파싱 라이브러리 사용
  
  const newJobConfig = `
  # ${config.serviceName} 모니터링
  - job_name: '${config.monitoring.prometheusJob}'
    static_configs:
      - targets: ['${config.monitoring.targets.join("', '")}']
        labels:
          instance: '${config.serviceName}'
          service: '${config.serviceName}'
          environment: 'production'
`;
  
  // 설정 파일에 추가 (실제 구현 필요)
  // ...
  
  // Prometheus 재시작
  await execPromise(`${sshCommand} "sudo docker restart prometheus"`);
  
  return {
    success: true,
    message: 'Prometheus Job이 추가되었습니다.'
  };
}

module.exports = {
  createAlertRule,
  addPrometheusJob
};
```

### 3. Prometheus 설정 확인

#### 현재 Prometheus 설정 요약
- **설정 파일**: `/mnt/plg-stack/prometheus/prometheus.yml`
- **Scrape Interval**: 15초
- **Evaluation Interval**: 15초
- **현재 모니터링 대상**:
  - `prometheus` (자체 모니터링)
  - `alertmanager` (Alertmanager 모니터링)
  - `loki` (Loki 모니터링)
  - `nginx-test-01` (10.255.1.253:9100)
  - `nginx-test-02` (10.255.1.102:9100)

#### 새 서비스 모니터링 추가 방법
```bash
# 1. 서버 접속
ssh -i '/Users/jhlee/Desktop/Work/Dana-Cloud-Oper/DanaIX-신규 VM 생성/00. SSH-PemKey-List/danainfra' ubuntu@10.255.1.254

# 2. Prometheus 설정 파일 수정
sudo vi /mnt/plg-stack/prometheus/prometheus.yml

# 3. 새 Job 추가 예시
# scrape_configs:
#   - job_name: 'web-server-service'
#     static_configs:
#       - targets: ['10.255.1.200:9100', '10.255.1.201:9100']
#         labels:
#           instance: 'web-server-service'
#           service: 'web-server'
#           environment: 'production'

# 4. Prometheus 재시작
sudo docker restart prometheus
```

### 4. Alertmanager Webhook 설정

#### 현재 Alertmanager 설정 (실제 서버)

**설정 파일 위치**: `/mnt/plg-stack/alertmanager/config/alertmanager.yml`

**현재 설정 내용**:
```yaml
global:
  resolve_timeout: 5m

route:
  group_by: ['alertname', 'cluster', 'service']
  group_wait: 10s
  group_interval: 10s
  repeat_interval: 12h
  receiver: 'jenkins-webhook'
  routes:
    - match:
        severity: critical
      receiver: 'jenkins-webhook'
      continue: true
    - match:
        severity: warning
      receiver: 'jenkins-webhook'

receivers:
  - name: 'jenkins-webhook'
    webhook_configs:
      - url: 'http://10.255.0.103:8080/generic-webhook-trigger/invoke?token=plg-autoscale-token'
        send_resolved: true
        http_config:
          follow_redirects: true
          basic_auth:
            username: 'danacloud'
            password: '!danacloud12'
```

#### Alertmanager 설정 수정 방법

```bash
# 1. 서버 접속
ssh -i '/Users/jhlee/Desktop/Work/Dana-Cloud-Oper/DanaIX-신규 VM 생성/00. SSH-PemKey-List/danainfra' ubuntu@10.255.1.254

# 2. 설정 파일 수정
sudo vi /mnt/plg-stack/alertmanager/config/alertmanager.yml

# 3. Alertmanager 재시작
sudo docker restart alertmanager
```

#### Jenkins Webhook 정보
- **Webhook URL**: `http://10.255.0.103:8080/generic-webhook-trigger/invoke?token=plg-autoscale-token`
- **인증 방식**: Basic Auth
  - **Username**: `danacloud`
  - **Password**: `!danacloud12`
- **Token**: `plg-autoscale-token`

#### Alertmanager 설정 자동화 (백엔드 서비스)

```javascript
/**
 * Alertmanager 설정에 새 라우팅 규칙 추가
 * @param {object} config - 오토스케일링 설정
 * @returns {Promise<object>} 설정 결과
 */
async function updateAlertmanagerConfig(config) {
  const sshCommand = `ssh -i "${PLG_STACK_SSH_KEY}" -o StrictHostKeyChecking=no ${PLG_STACK_USER}@${PLG_STACK_SERVER}`;
  const alertmanagerConfigPath = '/mnt/plg-stack/alertmanager/config/alertmanager.yml';
  
  // YAML 파일 읽기, 수정, 쓰기 (실제 구현 시 yaml 라이브러리 사용)
  // 여기서는 개념만 제공
  
  // 설정 파일 백업
  await execPromise(`${sshCommand} "sudo cp ${alertmanagerConfigPath} ${alertmanagerConfigPath}.backup"`);
  
  // 설정 파일 수정 (YAML 파싱 및 라우팅 규칙 추가)
  // ...
  
  // Alertmanager 재시작
  await execPromise(`${sshCommand} "sudo docker restart alertmanager"`);
  
  return {
    success: true,
    message: 'Alertmanager 설정이 업데이트되었습니다.'
  };
}
```

---

## Jenkins 파이프라인 구현

### 1. Jenkins 서버 정보

#### 멀티 서비스 파이프라인 아키텍처

**중요**: 각 서비스(고객)마다 독립적인 Jenkins 파이프라인이 필요합니다.

**이유**:
- IaaS 서비스 운영 환경에서 10~20개의 고객이 각각 독립적인 서비스를 운영
- 각 서비스의 스케일아웃이 서로 영향을 주지 않아야 함
- 서비스별로 다른 스케일링 정책 및 설정이 필요
- 독립적인 로그 관리 및 모니터링 필요

**Job 명명 규칙**:
- 형식: `autoscale-{service-name}` 또는 `autoscale-{customer-name}-{service-name}`
- 예시:
  - `autoscale-web-server-service`
  - `autoscale-customer-a-web-server`
  - `autoscale-customer-b-api-server`

#### 현재 구성된 Jenkins Jobs

**plg-autoscale-out** (기존 통합 파이프라인 - 참고용):
- **설명**: Alertmanager Webhook → Jenkins → vCenter API → F5 Pool 자동 등록 파이프라인
- **Webhook Token**: `plg-autoscale-token`
- **Webhook URL**: `http://10.255.0.103:8080/generic-webhook-trigger/invoke?token=plg-autoscale-token`
- **설정 파일**: `/var/lib/jenkins/jobs/plg-autoscale-out/config.xml`
- **참고**: 이 Job은 예시용이며, 실제로는 각 서비스마다 독립적인 Job을 생성해야 함

**vm-deployment-pipeline** (VM 배포 파이프라인):
- **설명**: Terraform을 사용한 VM 자동 배포 파이프라인
- **Git 저장소**: `https://github.com/zzambab98/vm-deployment-pipeline.git`
- **Jenkinsfile 경로**: `jenkins/Jenkinsfile`
- **파라미터**: `VM_ENVIRONMENT` (환경 이름)

#### 서비스별 독립 파이프라인 예시

**autoscale-web-server-service** (서비스 1):
- **Webhook Token**: `autoscale-web-server-service-token`
- **Webhook URL**: `http://10.255.0.103:8080/generic-webhook-trigger/invoke?token=autoscale-web-server-service-token`
- **설정 파일**: `/var/lib/jenkins/jobs/autoscale-web-server-service/config.xml`

**autoscale-api-server-service** (서비스 2):
- **Webhook Token**: `autoscale-api-server-service-token`
- **Webhook URL**: `http://10.255.0.103:8080/generic-webhook-trigger/invoke?token=autoscale-api-server-service-token`
- **설정 파일**: `/var/lib/jenkins/jobs/autoscale-api-server-service/config.xml`

#### Jenkins Job 설정 확인 및 수정

```bash
# 1. 서버 접속
ssh jenkins@10.255.0.103
# 비밀번호: !danacloud12

# 2. Job 설정 파일 확인
sudo cat /var/lib/jenkins/jobs/plg-autoscale-out/config.xml

# 3. Job 설정 파일 수정 (주의: 직접 수정 시 Jenkins 재시작 필요)
sudo vi /var/lib/jenkins/jobs/plg-autoscale-out/config.xml
sudo systemctl restart jenkins

# 4. Job 빌드 로그 확인
sudo tail -f /var/lib/jenkins/jobs/plg-autoscale-out/builds/{build-number}/log
```

### 2. Jenkinsfile 작성

#### 파일: `jenkins/Jenkinsfile.autoscale` (새로운 자동 스케일아웃 파이프라인)

```groovy
pipeline {
    agent any
    
    parameters {
        string(name: 'AUTOSCALE_CONFIG_ID', defaultValue: '', description: '오토스케일링 설정 ID')
        string(name: 'TEMPLATE_NAME', defaultValue: '', description: '템플릿 이름')
        string(name: 'F5_POOL_NAME', defaultValue: '', description: 'F5 Pool 이름')
        string(name: 'VIP', defaultValue: '', description: 'VIP 주소')
        string(name: 'VIP_PORT', defaultValue: '80', description: 'VIP 포트')
        string(name: 'IP_POOL_START', defaultValue: '', description: 'IP Pool 시작 주소')
        string(name: 'IP_POOL_END', defaultValue: '', description: 'IP Pool 종료 주소')
        string(name: 'VLAN', defaultValue: '', description: 'VLAN 이름')
    }
    
    stages {
        stage('Checkout') {
            steps {
                echo 'Checking out code...'
                // 필요시 코드 체크아웃
            }
        }
        
        stage('Get Available IP') {
            steps {
                script {
                    def ipRange = "${params.IP_POOL_START}-${params.IP_POOL_END}"
                    def availableIp = sh(
                        script: """
                            python3 scripts/get-available-ip.py \
                                --start ${params.IP_POOL_START} \
                                --end ${params.IP_POOL_END} \
                                --subnet ${params.VLAN}
                        """,
                        returnStdout: true
                    ).trim()
                    
                    env.VM_IP = availableIp
                    echo "할당된 IP: ${env.VM_IP}"
                }
            }
        }
        
        stage('Ping Test') {
            steps {
                script {
                    def pingResult = sh(
                        script: """
                            ping -c 3 -W 2 ${env.VM_IP} || exit 1
                        """,
                        returnStatus: true
                    )
                    
                    if (pingResult == 0) {
                        error("IP ${env.VM_IP}가 이미 사용 중입니다. 다른 IP를 선택하세요.")
                    }
                    
                    echo "IP ${env.VM_IP} 사용 가능 확인"
                }
            }
        }
        
        stage('Create VM from Template') {
            steps {
                script {
                    def vmName = "vm-${params.AUTOSCALE_CONFIG_ID}-${BUILD_NUMBER}"
                    
                    sh """
                        govc vm.clone \
                            -vm="${params.TEMPLATE_NAME}" \
                            -ds=datastore1 \
                            -folder=/DC/vm \
                            -net="${params.VLAN}" \
                            -ip="${env.VM_IP}" \
                            -netmask="255.255.255.0" \
                            -gateway="10.255.1.1" \
                            "${vmName}"
                    """
                    
                    env.VM_NAME = vmName
                    echo "VM 생성 완료: ${env.VM_NAME}"
                }
            }
        }
        
        stage('Wait for VM Boot') {
            steps {
                script {
                    echo "VM 부팅 대기 중..."
                    sleep(time: 60, unit: 'SECONDS')
                    
                    // VM이 부팅될 때까지 대기
                    def maxRetries = 30
                    def retryCount = 0
                    def vmReady = false
                    
                    while (retryCount < maxRetries && !vmReady) {
                        def pingResult = sh(
                            script: "ping -c 1 -W 2 ${env.VM_IP}",
                            returnStatus: true
                        )
                        
                        if (pingResult == 0) {
                            vmReady = true
                            echo "VM 부팅 완료: ${env.VM_IP}"
                        } else {
                            retryCount++
                            sleep(time: 10, unit: 'SECONDS')
                        }
                    }
                    
                    if (!vmReady) {
                        error("VM 부팅 실패: ${env.VM_IP}")
                    }
                }
            }
        }
        
        stage('Health Check') {
            steps {
                script {
                    echo "Health Check 수행 중..."
                    // 실제 Health Check 로직 구현
                    // 예: HTTP 요청, 서비스 상태 확인 등
                }
            }
        }
        
        stage('Add to F5 Pool') {
            steps {
                script {
                    sh """
                        python3 scripts/add-f5-pool-member.py \
                            --pool ${params.F5_POOL_NAME} \
                            --member ${env.VM_IP}:${params.VIP_PORT} \
                            --vip ${params.VIP}
                    """
                    echo "F5 Pool에 Member 추가 완료: ${env.VM_IP}:${params.VIP_PORT}"
                }
            }
        }
    }
    
    post {
        success {
            echo '스케일아웃 성공!'
            // 성공 알림 (Slack, Email 등)
        }
        failure {
            echo '스케일아웃 실패!'
            // 실패 알림 및 롤백
        }
    }
}
```

### 3. 서비스별 Jenkins Job 자동 생성

#### 오토스케일링 설정 시 자동 Job 생성

오토스케일링 설정을 생성할 때마다 해당 서비스에 대한 독립적인 Jenkins Job이 자동으로 생성됩니다.

#### 백엔드 서비스 구현 (Jenkins Job 자동 생성)

```javascript
// backend/src/services/jenkinsService.js

const axios = require('axios');
const { exec } = require('child_process');
const util = require('util');
const execPromise = util.promisify(exec);

const JENKINS_URL = process.env.JENKINS_URL || 'http://10.255.0.103:8080';
const JENKINS_USER = process.env.JENKINS_WEB_USER || 'danacloud';
const JENKINS_PASSWORD = process.env.JENKINS_WEB_PASSWORD || '!danacloud12';
const JENKINS_SERVER = process.env.JENKINS_SERVER || '10.255.0.103';
const JENKINS_SSH_USER = process.env.JENKINS_SSH_USER || 'jenkins';
const JENKINS_SSH_PASSWORD = process.env.JENKINS_SSH_PASSWORD || '!danacloud12';

/**
 * 서비스별 Jenkins Job 생성
 * @param {object} config - 오토스케일링 설정
 * @returns {Promise<object>} 생성 결과
 */
async function createJenkinsJob(config) {
  const jobName = config.jenkins.jobName || `autoscale-${config.serviceName.toLowerCase().replace(/\s+/g, '-')}`;
  const webhookToken = config.jenkins.webhookToken || `${jobName}-token`;
  
  // Jenkins Job XML 생성
  const jobXml = generateJobXml(jobName, webhookToken, config);
  
  // Jenkins API를 통해 Job 생성
  const createJobUrl = `${JENKINS_URL}/createItem?name=${encodeURIComponent(jobName)}`;
  const auth = Buffer.from(`${JENKINS_USER}:${JENKINS_PASSWORD}`).toString('base64');
  
  try {
    const response = await axios.post(createJobUrl, jobXml, {
      headers: {
        'Content-Type': 'application/xml',
        'Authorization': `Basic ${auth}`
      }
    });
    
    return {
      success: true,
      jobName: jobName,
      webhookToken: webhookToken,
      webhookUrl: `${JENKINS_URL}/generic-webhook-trigger/invoke?token=${webhookToken}`
    };
  } catch (error) {
    console.error(`[Jenkins Service] Job 생성 실패:`, error);
    throw new Error(`Jenkins Job 생성 실패: ${error.message}`);
  }
}

/**
 * Jenkins Job XML 생성
 */
function generateJobXml(jobName, webhookToken, config) {
  return `<?xml version='1.1' encoding='UTF-8'?>
<flow-definition plugin="workflow-job@2.45">
  <description>자동 스케일아웃 파이프라인 - ${config.serviceName}</description>
  <keepDependencies>false</keepDependencies>
  <properties>
    <org.jenkinsci.plugins.workflow.job.properties.PipelineTriggersJobProperty>
      <triggers>
        <org.jenkinsci.plugins.gwt.GenericTrigger plugin="generic-webhook-trigger@2.4.1">
          <spec></spec>
          <genericVariables>
            <org.jenkinsci.plugins.gwt.GenericVariable>
              <key>alertname</key>
              <value>\$.alerts[0].labels.alertname</value>
              <regexpFilter></regexpFilter>
            </org.jenkinsci.plugins.gwt.GenericVariable>
            <org.jenkinsci.plugins.gwt.GenericVariable>
              <key>instance</key>
              <value>\$.alerts[0].labels.instance</value>
              <regexpFilter></regexpFilter>
            </org.jenkinsci.plugins.gwt.GenericVariable>
            <org.jenkinsci.plugins.gwt.GenericVariable>
              <key>service</key>
              <value>\$.alerts[0].labels.service</value>
              <regexpFilter></regexpFilter>
            </org.jenkinsci.plugins.gwt.GenericVariable>
            <org.jenkinsci.plugins.gwt.GenericVariable>
              <key>autoscaleConfigId</key>
              <value>\$.alerts[0].labels.autoscaleConfigId</value>
              <regexpFilter></regexpFilter>
            </org.jenkinsci.plugins.gwt.GenericVariable>
            <org.jenkinsci.plugins.gwt.GenericVariable>
              <key>status</key>
              <value>\$.status</value>
              <regexpFilter></regexpFilter>
            </org.jenkinsci.plugins.gwt.GenericVariable>
          </genericVariables>
          <regexpFilterText>\$status</regexpFilterText>
          <regexpFilterExpression>^firing\$</regexpFilterExpression>
          <printPostContent>true</printPostContent>
          <printContributedVariables>true</printContributedVariables>
          <causeString>Alert: \$alertname - Service: \$service</causeString>
          <token>${webhookToken}</token>
          <silentResponse>false</silentResponse>
        </org.jenkinsci.plugins.gwt.GenericTrigger>
      </triggers>
    </org.jenkinsci.plugins.workflow.job.properties.PipelineTriggersJobProperty>
  </properties>
  <definition class="org.jenkinsci.plugins.workflow.cps.CpsScmFlowDefinition" plugin="workflow-cps@2.92">
    <scm class="hudson.plugins.git.GitSCM" plugin="git@4.11.4">
      <configVersion>2</configVersion>
      <userRemoteConfigs>
        <hudson.plugins.git.UserRemoteConfig>
          <url>https://github.com/zzambab98/vm-deployment-pipeline.git</url>
        </hudson.plugins.git.UserRemoteConfig>
      </userRemoteConfigs>
      <branches>
        <hudson.plugins.git.BranchSpec>
          <name>*/main</name>
        </hudson.plugins.git.BranchSpec>
      </branches>
      <doGenerateSubmoduleConfigurations>false</doGenerateSubmoduleConfigurations>
      <submoduleCfg class="list"/>
      <extensions/>
    </scm>
    <scriptPath>jenkins/Jenkinsfile.autoscale</scriptPath>
    <lightweight>true</lightweight>
  </definition>
  <disabled>false</disabled>
</flow-definition>`;
}

module.exports = {
  createJenkinsJob
};
```

#### Web UI를 통한 수동 Job 생성 (참고용)

1. Jenkins Web UI 접속: `http://10.255.0.103:8080/`
2. 로그인: `danacloud` / `!danacloud12`
3. "New Item" 클릭
4. Job 이름 입력 (예: `autoscale-web-server-service`)
5. "Pipeline" 선택
6. "OK" 클릭
7. Pipeline 설정:
   - **Definition**: Pipeline script from SCM
   - **SCM**: Git
   - **Repository URL**: `https://github.com/zzambab98/vm-deployment-pipeline.git`
   - **Script Path**: `jenkins/Jenkinsfile.autoscale`
8. Generic Webhook Trigger 설정:
   - "Build Triggers" 섹션에서 "Generic Webhook Trigger" 체크
   - **Token**: 서비스별 고유 토큰 입력 (예: `autoscale-web-server-service-token`)
   - **Post content parameters** 추가
9. "Save" 클릭

#### Alertmanager에서 서비스별 라우팅 설정

각 서비스별로 다른 Jenkins Job으로 라우팅하도록 Alertmanager 설정:

```yaml
# /mnt/plg-stack/alertmanager/config/alertmanager.yml

route:
  group_by: ['alertname', 'cluster', 'service']
  group_wait: 10s
  group_interval: 10s
  repeat_interval: 12h
  receiver: 'default-webhook'
  routes:
    # 서비스별 라우팅 규칙
    - match:
        service: web-server-service
      receiver: 'jenkins-webhook-web-server'
      continue: false
    - match:
        service: api-server-service
      receiver: 'jenkins-webhook-api-server'
      continue: false
    # 기본 라우팅 (기존 서비스)
    - match:
        severity: critical
      receiver: 'default-webhook'
      continue: true
    - match:
        severity: warning
      receiver: 'default-webhook'

receivers:
  # 서비스별 Webhook 수신자
  - name: 'jenkins-webhook-web-server'
    webhook_configs:
      - url: 'http://10.255.0.103:8080/generic-webhook-trigger/invoke?token=autoscale-web-server-service-token'
        send_resolved: true
        http_config:
          follow_redirects: true
          basic_auth:
            username: 'danacloud'
            password: '!danacloud12'
  
  - name: 'jenkins-webhook-api-server'
    webhook_configs:
      - url: 'http://10.255.0.103:8080/generic-webhook-trigger/invoke?token=autoscale-api-server-service-token'
        send_resolved: true
        http_config:
          follow_redirects: true
          basic_auth:
            username: 'danacloud'
            password: '!danacloud12'
  
  # 기본 Webhook (기존 서비스용)
  - name: 'default-webhook'
    webhook_configs:
      - url: 'http://10.255.0.103:8080/generic-webhook-trigger/invoke?token=plg-autoscale-token'
        send_resolved: true
        http_config:
          follow_redirects: true
          basic_auth:
            username: 'danacloud'
            password: '!danacloud12'
```

#### Jenkins API를 통한 Job 실행 (테스트용)
```bash
# 서비스별 Webhook을 통한 자동 실행 (Alertmanager에서 호출)
# Web Server Service
curl -X POST \
  "http://10.255.0.103:8080/generic-webhook-trigger/invoke?token=autoscale-web-server-service-token" \
  -H "Content-Type: application/json" \
  -d '{
    "status": "firing",
    "alerts": [{
      "labels": {
        "alertname": "HighCPU",
        "instance": "10.255.1.100:9100",
        "service": "web-server-service",
        "autoscaleConfigId": "autoscale-001",
        "severity": "critical"
      },
      "annotations": {
        "summary": "CPU usage is above 80%",
        "description": "CPU usage is 85% for 5 minutes"
      }
    }]
  }'

# API Server Service
curl -X POST \
  "http://10.255.0.103:8080/generic-webhook-trigger/invoke?token=autoscale-api-server-service-token" \
  -H "Content-Type: application/json" \
  -d '{
    "status": "firing",
    "alerts": [{
      "labels": {
        "alertname": "HighCPU",
        "instance": "10.255.1.200:9100",
        "service": "api-server-service",
        "autoscaleConfigId": "autoscale-002",
        "severity": "critical"
      },
      "annotations": {
        "summary": "CPU usage is above 80%",
        "description": "CPU usage is 85% for 5 minutes"
      }
    }]
  }'

# 수동 실행 (파라미터 포함)
curl -X POST \
  -u danacloud:!danacloud12 \
  "http://10.255.0.103:8080/job/vm-autoscale-out/buildWithParameters?token=plg-autoscale-token" \
  --data-urlencode "AUTOSCALE_CONFIG_ID=autoscale-001" \
  --data-urlencode "TEMPLATE_NAME=Web-Server-Template" \
  --data-urlencode "F5_POOL_NAME=web-server-pool" \
  --data-urlencode "VIP=10.255.1.100" \
  --data-urlencode "VIP_PORT=80" \
  --data-urlencode "IP_POOL_START=10.255.1.200" \
  --data-urlencode "IP_POOL_END=10.255.1.250" \
  --data-urlencode "VLAN=Vlan-101"
```

### 4. Jenkins Credential 설정

#### Credential 파일 위치
- **Credentials 설정**: `/var/lib/jenkins/credentials.xml`
- **Secrets 디렉토리**: `/var/lib/jenkins/secrets/`

#### 주요 Credential (예상)
- **vCenter 인증 정보**: `vcenter-server`, `vcenter-user`, `vcenter-password`
- **F5 인증 정보**: `f5-credentials`
- **GitHub 인증 정보**: `github-credentials` 또는 `github-token`
- **SSH 키**: `vm-ssh-key`, `vm-ssh-public-key`

#### Credential 확인 방법
```bash
# Credential 파일 확인 (암호화되어 있음)
sudo cat /var/lib/jenkins/credentials.xml

# Web UI에서 확인 권장
# Manage Jenkins → Manage Credentials → System → Global credentials
```

---

## F5 자동 연동 구현

### 1. F5 Pool Member 추가 스크립트

#### 파일: `scripts/add-f5-pool-member.py`

```python
#!/usr/bin/env python3
import requests
import json
import sys
import argparse
import os

F5_SERVERS = os.getenv('F5_SERVERS', '10.255.1.80').split(',')
F5_USERNAME = os.getenv('F5_USERNAME', 'admin')
F5_PASSWORD = os.getenv('F5_PASSWORD', '')
F5_PARTITION = 'Common'

def authenticate(server):
    """F5 인증 및 토큰 획득"""
    url = f'https://{server}/mgmt/shared/authn/login'
    payload = {
        'username': F5_USERNAME,
        'password': F5_PASSWORD,
        'loginProviderName': 'tmos'
    }
    
    response = requests.post(url, json=payload, verify=False)
    response.raise_for_status()
    
    return response.json()['token']['token']

def add_pool_member(server, token, pool_name, member):
    """F5 Pool에 Member 추가"""
    url = f'https://{server}/mgmt/tm/ltm/pool/~{F5_PARTITION}~{pool_name}/members'
    headers = {
        'X-F5-Auth-Token': token,
        'Content-Type': 'application/json'
    }
    
    # Member 형식: "IP:Port"
    member_data = {
        'name': member,
        'partition': F5_PARTITION
    }
    
    response = requests.post(url, headers=headers, json=member_data, verify=False)
    
    if response.status_code == 200 or response.status_code == 201:
        print(f'[F5] Pool Member 추가 성공: {member}')
        return True
    elif response.status_code == 409:
        print(f'[F5] Pool Member가 이미 존재합니다: {member}')
        return True
    else:
        print(f'[F5] Pool Member 추가 실패: {response.status_code} - {response.text}')
        return False

def main():
    parser = argparse.ArgumentParser(description='F5 Pool에 Member 추가')
    parser.add_argument('--pool', required=True, help='Pool 이름')
    parser.add_argument('--member', required=True, help='Member (IP:Port 형식)')
    parser.add_argument('--vip', required=True, help='VIP 주소')
    
    args = parser.parse_args()
    
    # Primary F5 서버만 사용
    primary_server = F5_SERVERS[0]
    
    try:
        # 인증
        token = authenticate(primary_server)
        
        # Pool Member 추가
        success = add_pool_member(primary_server, token, args.pool, args.member)
        
        if success:
            print(f'[Success] F5 Pool Member 추가 완료: {args.member}')
            sys.exit(0)
        else:
            print(f'[Error] F5 Pool Member 추가 실패: {args.member}')
            sys.exit(1)
            
    except Exception as e:
        print(f'[Error] F5 연동 실패: {str(e)}')
        sys.exit(1)

if __name__ == '__main__':
    main()
```

---

## IP Pool 관리 구현

### 1. 사용 가능한 IP 조회 스크립트

#### 파일: `scripts/get-available-ip.py`

```python
#!/usr/bin/env python3
import ipaddress
import subprocess
import argparse
import sys

def ping_test(ip):
    """IP Ping 테스트"""
    try:
        result = subprocess.run(
            ['ping', '-c', '1', '-W', '2', str(ip)],
            capture_output=True,
            timeout=5
        )
        return result.returncode == 0
    except:
        return False

def get_available_ip(start_ip, end_ip, subnet=None):
    """사용 가능한 IP 찾기"""
    start = ipaddress.IPv4Address(start_ip)
    end = ipaddress.IPv4Address(end_ip)
    
    # IP 범위 내에서 사용 가능한 IP 찾기
    for ip in ipaddress.summarize_address_range(start, end):
        for addr in ip:
            if addr == start or addr == end:
                continue
            
            # Ping 테스트
            if not ping_test(addr):
                return str(addr)
    
    return None

def main():
    parser = argparse.ArgumentParser(description='사용 가능한 IP 조회')
    parser.add_argument('--start', required=True, help='시작 IP')
    parser.add_argument('--end', required=True, help='종료 IP')
    parser.add_argument('--subnet', help='서브넷 (선택)')
    
    args = parser.parse_args()
    
    available_ip = get_available_ip(args.start, args.end, args.subnet)
    
    if available_ip:
        print(available_ip)
        sys.exit(0)
    else:
        print('[Error] 사용 가능한 IP를 찾을 수 없습니다.', file=sys.stderr)
        sys.exit(1)

if __name__ == '__main__':
    main()
```

---

## 다음 단계

1. **백엔드 API 구현 완료**
2. **프론트엔드 UI 구현**
3. **Jenkins 파이프라인 테스트**
4. **PLG Stack 연동 테스트**
5. **전체 워크플로우 통합 테스트**

---

**작성자**: Dana Cloud Automation Team  
**최종 수정일**: 2025-11-20

