...
1. **SSH 키 경로 전달 문제**
   ...
   - 원인: Generic Webhook Trigger에서 변수 파싱 실패 가능성
   - 조치 필요:
     - Jenkins Job의 Generic Webhook Trigger 설정 확인
     - Variables에서 `SSH_KEY_PATH`와 `VM_PREFIX`가 올바르게 설정되었는지 확인
     - Jenkins 서버에서 SSH 키 경로 접근 가능 여부 확인

2. **Fallback SSH 키 검색 실패**
   - 증상: Jenkins 서버에서 `/home/ubuntu/workspace/vm-autoscaling/pemkey/` 경로 접근 불가
   - 원인: Jenkins 서버와 백엔드 서버의 경로 차이
   - 조치 필요:
     - Jenkins 서버에서 실제 SSH 키 경로 확인
     - Jenkinsfile의 Fallback 경로 수정 또는 Jenkins Credentials 사용

#### 🔧 조치 필요 사항

1. **Jenkins Generic Webhook Trigger 설정 확인**
   - Variables에서 `SSH_KEY_PATH`와 `VM_PREFIX` 파라미터 확인
   - JSON Path가 올바른지 확인 (`$.config.sshKeyPath`, `$.config.vmPrefix`)

2. **Jenkins 서버 SSH 키 경로 확인**
   ```bash
   # Jenkins 서버에서 실행
   ls -la /home/ubuntu/workspace/vm-autoscaling/pemkey/
   # 또는 Jenkins 워크스페이스 경로 확인
   echo $WORKSPACE
   ```

3. **Webhook 테스트**
   - Alert 발생 시 Jenkins Console에서 환경 변수 값 확인
   - `🐛 DEBUG: env.SSH_KEY_PATH` 로그 확인
